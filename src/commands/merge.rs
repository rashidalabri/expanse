...
//! which keeps each contributing sample's own IRR count and motif
//! breakdown.
//!
...
/// One entry in the merge output: a locus formed by merging one or more
/// samples' anchor regions that fall within `--merge-distance` bp of each
/// other, keyed by sample id.
#[derive(Serialize, Debug)]
struct MergedLocusSummary {
    chrom: String,
    start: i64,
    end: i64,
...
pub fn run(args: MergeArgs) -> Result<()> {
    ...
    let mut merged: Vec<MergedLocusSummary> = Vec::new();
    ...
    for (batch_idx, batch) in manifest_entries.chunks(args.batch_size).enumerate() {
        ...
        let mut next_merged: Vec<MergedLocusSummary> = clusters
            .iter()
            .map(|region| MergedLocusSummary {
    ...
    let json =
        serde_json::to_string_pretty(&merged).context("failed to serialize merged locus summary")?;
    std::fs::write(&args.output, json)
        .with_context(|| format!("failed to write merged summary {:?}", args.output))?;

    log::info!(
        "merge: {} samples from {:?} merged into {} loci, written to {:?}",
